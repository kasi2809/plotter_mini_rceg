...
def select_port():
    ...
    if (i_s == 1):
        ...
        selp_window.iconbitmap('icon2.ico')
...
def proced():
    ...
    # global file_path, brd_count, points, port_selected
    # points = []
    # global reset
    # if entry_1.get() == '' or myCombo.get() == "--Count--":
    #     messagebox.showinfo(title="Warning", message="Please fill all the entrys!")
    ...
    #     brd_count_local = brd_count
    #     num_of_points_local = len(points)
    #     ###############################
    #     info = "Bill:" + "\nNumber of holes:" + str(num_of_points_local) \
    #            + "\nNumber of boards:" + str(brd_count_local) \
    #            + "\nCost per board:" + str(num_of_points_local * 2.00) \
    #            + "\nTotal Cost (INR): " + str(brd_count_local * num_of_points_local * 2.00) \
    #            + "\n\nWant to continue?"
    #     if (messagebox.askokcancel("Bill Confirmation", info)):
    #         enter_data(file_path_local, num_of_points_local, brd_count_local)
    #         print('Value appended')
    #         cnc_pcb_v1_1.send_data("4" + "," + str(brd_count) + "," + str(len(points)))
    #         sorted_points = cnc_pcb_v1_1.find_near_pts(points)
    #         print("Coordinates sorted")
...
def abt():
    messagebox.showinfo(title="About",
                        message="This is a GUI Application done for the PCB soldering machine.\n Icon Credit:https://free-icon-rainbow.com/soldering-iron-icon-3/")
...
def callibrate_win():
    ...
    if (i_c == 1):
        ...
        clb_window.iconbitmap('icon2.ico')
        clb_window.resizable(width=False, height=False)
        clb_window.geometry("500x300")
        clb_window.config(bg="white")
        # Up Button
        up_bn = PhotoImage(file='up.png')
        up_btn = Button(clb_window, text="x", font=("arial", 12, "bold"), image=up_bn, compound="center",
                        relief="groove", command=up)
        up_btn.place(x=100, y=30)
        # Left Button
        left_bn = PhotoImage(file='left.png')
        left_btn = Button(clb_window, text="-y", font=("arial", 12, "bold"), image=left_bn, compound="center",
                          relief="groove", command=left)
        left_btn.place(x=30, y=100)
        # Down Button
        down_bn = PhotoImage(file='down.png')
        down_btn = Button(clb_window, text="-x", font=("arial", 12, "bold"), image=down_bn, compound="center",
                          relief="groove", command=down)
        down_btn.place(x=100, y=170)
        # Right Button
        right_bn = PhotoImage(file='right.png')
        right_btn = Button(clb_window, text="y", font=("arial", 12, "bold"), image=right_bn, compound="center",
                           relief="groove", command=right)
        right_btn.place(x=170, y=100)
        # Z Up Button
        z_up_bn = PhotoImage(file='z_up.png')
        z_up_btn = Button(clb_window, text="z", font=("arial", 12, "bold"), fg="white", image=z_up_bn,
                          compound="center", relief="groove", command=z_up)
        z_up_btn.place(x=320, y=30)
        # Z Down Button
        z_down_bn = PhotoImage(file='z_down.png')
        z_down_btn = Button(clb_window, text="-z", fg="white", font=("arial", 12, "bold"), image=z_down_bn,
                            compound="center", relief="groove", command=z_down)
        z_down_btn.place(x=320, y=170)
        # start
        solder_bn = PhotoImage(file='solder.png')
